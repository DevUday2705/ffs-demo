...
export default function AppliedJobs({ isOpen, onClose, appliedData = [] }) {
  ...
  <div className="max-w-2xl mx-auto">
    ...
    <DialogHeader className="mb-6 text-center">
      <DialogTitle className="flex justify-center items-center gap-2 text-xl font-semibold text-gray-900">
          ...
          >
            ...
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  JR ID:{" "}
                  <span className="font-medium text-gray-700">{job.jr_id}</span>
                </p>
              </div>

              <span
                className={`px-3 py-1 text-xs font-semibold rounded-full capitalize ${
                  (job.status || "Invited") === "Selected"
                        ? "bg-green-100 text-green-700"
                        : (job.status || "Invited") === "Rejected"
                        ? "bg-red-100 text-red-700"
                        : (job.status || "Invited") === "Interview"
                        ? "bg-blue-100 text-blue-700"
                        : (job.status || "Invited") === "Screening"
            ...
            <div className="mt-4">
              <p className="text-sm text-gray-600">
                <span className="font-medium capitalize text-gray-700">Remarks:</span>{" "}
            ...
            {/* Description */}
            {/* {job.job_description && (
              <div className="mt-4 text-sm text-gray-600 leading-relaxed line-clamp-5">
                {job.job_description}
              </div>
            )} */}
          </div>
